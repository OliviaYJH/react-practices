...
public class ItemController {
	...
	public ResponseEntity<JsonResult<Item>> create(Item item, MultipartFile file) {
		log.info("Request[POST /api, Content-Type: multipart/form-data][{}, {}]", item, file.getOriginalFilename());
		...
		try {
			...
			Files.write(Files
					.createDirectories(Paths.get("/Users/yujunghyun/Documents/poscodx/ajax-practices-uploads/images"))
					.resolve(saveFileName), file.getBytes());

			Long maxId = Optional.ofNullable(items.isEmpty() ? null : items.getFirst()).map(t -> t.getId()).orElse(0L);
